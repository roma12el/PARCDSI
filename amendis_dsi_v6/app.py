...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with c1:
        if "direction" in df_chrom.columns:
            ...
            fig.update_layout(yaxis=dict(tickmode="linear", automargin=True),
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    # FIX: supprimer les fausses directions ("Oui", "OUI/NON", "DIRECTION", etc.)
    ...
    if "type_pc" in mouvt_clean.columns:
        def _norm_type(v):
            s = str(v).upper().strip()
            if s in ("BUREAU","PC BUREAU","FIXE","DESKTOP"): return "BUREAU"
            if s in ("PORTABLE","LAPTOP","NOTEBOOK"): return "PORTABLE"
            if s in ("STATION","WORKSTATION"): return "STATION"
            if s in ("CHROMEBOOK","CHROME"): return "CHROMEBOOK"
            if s in VALID_TYPES: return s
            return "BUREAU"
    ...
    with c1:
        if "direction" in mouvt_clean.columns:
            ...
            fig_d.update_layout(yaxis=dict(tickmode="linear", automargin=True),
                                xaxis=dict(showgrid=True, gridcolor="#F0F4F8"))
            st.plotly_chart(_fig(fig_d,400), use_container_width=True)
    with c2:
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    if df_pc_raw is None:
        st.warning("Chargez d'abord le fichier inventaire PC.")
        st.stop()
    ...
    st.markdown("""<div style="background:linear-gradient(135deg,#1B2A3B,#2D3E50); border-radius:16px; padding:24px 28px; margin-bottom:20px; color:white;">
        ...
        <div style="font-size:13px; color:#94A3B8;">Analyse par profil PC · direction · ancienneté · RAM · priorité</div>
    </div>""", unsafe_allow_html=True)

    # ── Filtres de segmentation ─────────────────────────────────────────────
    st.markdown('<div class="rpt-header">🎛️ Critères de Segmentation</div>', unsafe_allow_html=True)
    col_f1, col_f2, col_f3, col_f4 = st.columns(4)

    with col_f1:
        if "direction" in df_seg.columns:
            dirs_all = ["Toutes"] + sorted(df_seg["direction"].dropna().unique().tolist())
            seg_dir = st.selectbox("Direction", dirs_all, key="seg_dir")
        else:
            seg_dir = "Toutes"

    with col_f2:
        if "priorite" in df_seg.columns:
            prios_all = ["Toutes"] + sorted(df_seg["priorite"].dropna().unique().tolist())
            seg_prio = st.selectbox("Priorité DSI", prios_all, key="seg_prio")
        else:
            seg_prio = "Toutes"

    with col_f3:
        if "categorie_age" in df_seg.columns:
    ...
    with col_f4:
        if "type_pc" in df_seg.columns:
            types_all = ["Tous"] + sorted(df_seg["type_pc"].dropna().unique().tolist())
            seg_type = st.selectbox("Type PC", types_all, key="seg_type")
        else:
            seg_type = "Tous"

    # Appliquer filtres
    df_filtered = df_seg.copy()
    if seg_dir  != "Toutes" and "direction"    in df_filtered.columns: df_filtered = df_filtered[df_filtered["direction"]    == seg_dir]
    if seg_prio != "Toutes" and "priorite"     in df_filtered.columns: df_filtered = df_filtered[df_filtered["priorite"]     == seg_prio]
    if seg_age  != "Toutes" and "categorie_age" in df_filtered.columns: df_filtered = df_filtered[df_filtered["categorie_age"] == seg_age]
    if seg_type != "Tous"   and "type_pc"       in df_filtered.columns: df_filtered = df_filtered[df_filtered["type_pc"]       == seg_type]

    n_seg = len(df_filtered)
    n_total = len(df_seg)
    pct_seg = round(n_seg / n_total * 100) if n_total > 0 else 0

    # KPIs du segment
    st.markdown("<br>", unsafe_allow_html=True)
    c1,c2,c3,c4,c5 = st.columns(5)
    urg_s  = df_filtered[df_filtered["priorite"]=="🔴 Urgent"].shape[0]  if "priorite" in df_filtered.columns else 0
    ok_s   = df_filtered[df_filtered["priorite"]=="🟢 Opérationnel"].shape[0] if "priorite" in df_filtered.columns else 0
    avg_ram= df_filtered["ram_go"].mean() if "ram_go" in df_filtered.columns and df_filtered["ram_go"].notna().any() else 0
    avg_age_s= df_filtered["anciennete"].mean() if "anciennete" in df_filtered.columns and df_filtered["anciennete"].notna().any() else 0
    ...
    kpi(c4,"💾","RAM Moy.",      f"{avg_ram:.0f} GO", "#2980B9")
    kpi(c5,"📅","Âge Moy.",      f"{avg_age_s:.1f} ans", "#1B2A3B")

    st.markdown("<br>", unsafe_allow_html=True)

    if n_seg == 0:
        st.info("Aucun utilisateur ne correspond à ces critères de segmentation.")
        st.stop()

    # ── Graphiques de segmentation ──────────────────────────────────────────
    rpt_hdr("📊 Profil du Segment Sélectionné")
    c1, c2 = st.columns(2)

    with c1:
        if "direction" in df_filtered.columns:
            cnt_d = df_filtered.groupby("direction").size().reset_index(name="count").sort_values("count")
            fig_d = px.bar(cnt_d, x="count", y="direction", orientation="h",
                           title="Utilisateurs par Direction",
                           color="count", color_continuous_scale=[[0,"#F5A0AE"],[1,"#C8102E"]], text="count")
            fig_d.update_coloraxes(showscale=False); fig_d.update_traces(textposition="outside")
            fig_d.update_layout(yaxis=dict(tickmode="linear", automargin=True))
            st.plotly_chart(_fig(fig_d, 380), use_container_width=True)

    with c2:
        if "priorite" in df_filtered.columns:
            cnt_p = df_filtered["priorite"].value_counts().reset_index(); cnt_p.columns=["prio","count"]
            clr_p = {"🔴 Urgent":"#C8102E","🟠 Prioritaire":"#E67E22","🟡 À surveiller":"#F1C40F","🟢 Opérationnel":"#27AE60"}
            fig_p = px.pie(cnt_p, values="count", names="prio", title="Répartition Priorités",
                           color="prio", color_discrete_map=clr_p, hole=0.52)
            fig_p.update_traces(textposition="inside", textinfo="percent+label")
            st.plotly_chart(_fig(fig_p, 380), use_container_width=True)

    c1, c2, c3 = st.columns(3)
    with c1:
        if "categorie_age" in df_filtered.columns:
            order=["Récent (≤3 ans)","3-5 ans","5-10 ans","Plus de 10 ans","Inconnu"]
            clr_a={"Récent (≤3 ans)":"#27AE60","3-5 ans":"#2980B9","5-10 ans":"#E67E22","Plus de 10 ans":"#C8102E","Inconnu":"#94A3B8"}
            cnt_a=df_filtered["categorie_age"].value_counts().reindex(order, fill_value=0).reset_index(); cnt_a.columns=["cat","count"]
            fig_a=px.bar(cnt_a, x="cat", y="count", title="Ancienneté PC",
                         color="cat", color_discrete_map=clr_a, text="count")
            fig_a.update_layout(showlegend=False); fig_a.update_traces(textposition="outside")
            st.plotly_chart(_fig(fig_a, 300), use_container_width=True)

    with c2:
        if "ram_go" in df_filtered.columns:
            r = df_filtered["ram_go"].dropna().value_counts().sort_index().reset_index(); r.columns=["ram","count"]
            r["label"] = r["ram"].apply(lambda x: f"{int(x)} GO")
            fig_r = px.bar(r, x="label", y="count", title="Distribution RAM",
                           color="count", color_continuous_scale=[[0,"#F5A0AE"],[1,"#C8102E"]], text="count")
            fig_r.update_coloraxes(showscale=False); fig_r.update_traces(textposition="outside")
            st.plotly_chart(_fig(fig_r, 300), use_container_width=True)

    with c3:
        if "type_pc" in df_filtered.columns:
            cnt_t = df_filtered["type_pc"].value_counts().reset_index(); cnt_t.columns=["type","count"]
            fig_t = px.pie(cnt_t, values="count", names="type", title="Type de PC",
                           color_discrete_sequence=["#C8102E","#2980B9","#27AE60","#E67E22"], hole=0.48)
            fig_t.update_traces(textposition="inside", textinfo="percent+label")
            st.plotly_chart(_fig(fig_t, 300), use_container_width=True)

    # ── Tableau des utilisateurs du segment ─────────────────────────────────
    rpt_hdr("📋 Liste des Utilisateurs du Segment")
    sc_u = [c for c in ["utilisateur","direction","site","type_pc","annee_acquisition",
                         "anciennete","ram","processeur","priorite","recommandation"] if c in df_filtered.columns]
    search_seg = st.text_input("🔍 Rechercher dans ce segment", key="search_seg")
    df_show = df_filtered[sc_u].copy()
    if search_seg:
        mask_s = pd.Series(False, index=df_show.index)
        for col in df_show.columns:
            mask_s |= df_show[col].astype(str).str.contains(search_seg, case=False, na=False)
        df_show = df_show[mask_s]

    st.dataframe(df_show, use_container_width=True, height=420)
    st.caption(f"👥 {len(df_show)} utilisateur(s) dans ce segment")
    st.download_button("⬇️ Exporter le Segment Excel",
                       data=export_excel(df_show, "Segment"),
                       file_name=f"segment_{datetime.now().strftime('%Y%m%d')}.xlsx",
                       mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")


# ══════════════════════════════════════════════════════════════════════════════
# PAGE: DASHBOARD HTML INTÉGRÉ
# ══════════════════════════════════════════════════════════════════════════════
elif page == "📊 Dashboard HTML":
    if df_pc_raw is None and df_imp is None:
        st.warning("Chargez d'abord vos fichiers inventaire.")
        st.stop()

    df_d = df_pc_f if df_pc_f is not None else pd.DataFrame()

    # Calcul des stats pour le dashboard
    total_pc   = len(df_d)
    ...
    ok_n       = int(df_d[df_d["priorite"]=="🟢 Opérationnel"].shape[0])     if "priorite"  in df_d.columns else 0
    ...
    imp_n      = len(df_imp_f) if df_imp_f is not None else 0
    avg_age_d  = round(df_d["anciennete"].mean(), 1) if "anciennete" in df_d.columns and df_d["anciennete"].notna().any() else 0
    old_n      = int(df_d[df_d["categorie_age"]=="Plus de 10 ans"].shape[0]) if "categorie_age" in df_d.columns else 0
    nb_dir_d   = df_d["direction"].nunique() if "direction" in df_d.columns else 0

    # Données directions pour le tableau
    dir_rows_html = ""
    if "direction" in df_d.columns and "priorite" in df_d.columns:
        ...
        agg_d = agg_d.merge(urg_d, on="direction", how="left").fillna(0)
        agg_d["Urgents"] = agg_d["Urgents"].astype(int)
        agg_d = agg_d.sort_values("Total", ascending=False)
        for _, r2 in agg_d.head(12).iterrows():
            pct = round(r2["Total"] / total_pc * 100) if total_pc else 0
            urg_cls = "color:#C8102E; font-weight:700;" if r2["Urgents"] > 0 else "color:#27AE60;"
            dir_rows_html += f"""<tr>
              <td style="padding:10px 14px; font-weight:600; color:#1B2A3B;">{r2['direction']}</td>
              <td style="padding:10px 14px; text-align:center;">{int(r2['Total'])}</td>
              <td style="padding:10px 14px;">
                <div style="background:#F2F2F2; border-radius:4px; height:8px; overflow:hidden;">
                  <div style="background:#C8102E; width:{pct}%; height:100%; border-radius:4px;"></div>
                </div>
                <span style="font-size:11px; color:#64748B;">{pct}%</span>
              </td>
              <td style="padding:10px 14px; text-align:center; {urg_cls}">{int(r2['Urgents'])}</td>
            </tr>"""

    # Top processeurs
    proc_html = ""
    if "processeur" in df_d.columns:
        df_proc = df_d.copy(); df_proc["proc_cat"] = df_proc["processeur"].apply(cat_proc)
        cnt_proc = df_proc["proc_cat"].value_counts().head(6)
        max_p = cnt_proc.max() if len(cnt_proc)>0 else 1
        clr_proc = {"Core i7/Ultra7":"#C8102E","Core i5/Ultra5":"#2980B9","Core i3":"#27AE60",
                    "Celeron/Dual":"#E67E22","Xeon":"#9B59B6","Autre":"#94A3B8"}
        for proc_n, proc_c in cnt_proc.items():
            pct_p = round(proc_c/max_p*100)
            col_p = clr_proc.get(proc_n, "#94A3B8")
            proc_html += f"""<div style="margin-bottom:12px;">
              <div style="display:flex; justify-content:space-between; margin-bottom:4px;">
                <span style="font-size:13px; font-weight:600; color:#1B2A3B;">{proc_n}</span>
                <span style="font-size:13px; font-weight:700; color:{col_p};">{proc_c}</span>
              </div>
              <div style="background:#F2F2F2; border-radius:6px; height:10px; overflow:hidden;">
                <div style="background:{col_p}; width:{pct_p}%; height:100%; border-radius:6px; transition:width .6s ease;"></div>
              </div>
            </div>"""
    ...
    if "ram_go" in df_d.columns:
        ram_counts = df_d["ram_go"].dropna().value_counts().sort_index()
        total_ram = ram_counts.sum()
        clr_ram = {4:"#E8415B", 8:"#2980B9", 16:"#27AE60", 32:"#9B59B6"}
        for rv, rc in ram_counts.items():
            pct_r = round(rc/total_ram*100) if total_ram else 0
            col_r = clr_ram.get(int(rv), "#94A3B8")
            ram_html += f"""<div style="display:flex; align-items:center; gap:10px; margin-bottom:10px;">
              <div style="width:52px; font-size:13px; font-weight:700; color:{col_r};">{int(rv)} GO</div>
              <div style="flex:1; background:#F2F2F2; border-radius:6px; height:12px; overflow:hidden;">
                <div style="background:{col_r}; width:{pct_r}%; height:100%; border-radius:6px;"></div>
              </div>
              <div style="width:52px; text-align:right; font-size:13px; font-weight:600; color:#64748B;">{rc} ({pct_r}%)</div>
            </div>"""

    # Urgents list
    urg_list_html = ""
    if "priorite" in df_d.columns and urgent_n > 0:
        df_urg = df_d[df_d["priorite"]=="🔴 Urgent"].head(8)
        for _, ur in df_urg.iterrows():
            u_nom = val_str(ur.get("utilisateur")); u_dir = val_str(ur.get("direction"))
            u_age = val_str(ur.get("anciennete")); u_mod = val_str(ur.get("descriptions",""))[:28]
            urg_list_html += f"""<div style="display:flex; align-items:center; gap:12px; padding:10px 0; border-bottom:1px solid #F2F2F2;">
              ...
              <div style="flex:1; min-width:0;">
                <div style="font-weight:700; font-size:13px; color:#1B2A3B; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">{u_nom}</div>
                <div style="font-size:11px; color:#64748B;">{u_dir} · {u_mod}</div>
              </div>
              <div style="font-size:12px; font-weight:700; color:#C8102E; white-space:nowrap;">{u_age} ans</div>
            </div>"""
...
<style>
  * {{ box-sizing:border-box; margin:0; padding:0; font-family:'Segoe UI',system-ui,sans-serif; }}
  ...
  .card {{ background:white; border-radius:16px; padding:20px 22px; box-shadow:0 2px 12px rgba(0,0,0,.06); }}
  .card-title {{ font-size:13px; font-weight:700; color:#1B2A3B; text-transform:uppercase; letter-spacing:.5px; margin-bottom:16px; padding-bottom:10px; border-bottom:2px solid #F2F2F2; }}
  .kpi-big {{ font-size:38px; font-weight:900; line-height:1; }}
  .kpi-lbl {{ font-size:11px; font-weight:600; color:#64748B; text-transform:uppercase; letter-spacing:.5px; margin-top:5px; }}
  .kpi-sub {{ font-size:12px; color:#94A3B8; margin-top:3px; }}
  .kpi-card {{ background:white; border-radius:14px; padding:18px 20px; box-shadow:0 2px 10px rgba(0,0,0,.06); border-left:5px solid #C8102E; }}
  .badge {{ display:inline-block; padding:3px 10px; border-radius:20px; font-size:11px; font-weight:700; }}
  .badge-red {{ background:#FEF2F2; color:#C8102E; }}
  .badge-green {{ background:#F0FDF4; color:#27AE60; }}
  .badge-blue {{ background:#EFF6FF; color:#2980B9; }}
  .badge-orange {{ background:#FFFBEB; color:#E67E22; }}
  table {{ width:100%; border-collapse:collapse; }}
  th {{ background:#1B2A3B; color:white; padding:10px 14px; font-size:11px; font-weight:700; text-align:left; text-transform:uppercase; letter-spacing:.5px; }}
  tr:nth-child(even) {{ background:#FAFBFC; }}
  tr:hover {{ background:#FFF5F7; }}
  .header-bar {{ background:linear-gradient(135deg,#1B2A3B,#2D3E50); border-radius:16px; padding:22px 28px; color:white; margin-bottom:16px; display:flex; justify-content:space-between; align-items:center; }}
  ...
  .stat-pill {{ background:rgba(200,16,46,.15); border:1px solid rgba(200,16,46,.3); color:#F5A0AE; padding:4px 12px; border-radius:20px; font-size:11px; font-weight:600; }}
  .donut-wrap {{ display:flex; align-items:center; justify-content:center; gap:20px; flex-wrap:wrap; padding:8px 0; }}
  .donut-item {{ text-align:center; }}
  .donut-circle {{ width:72px; height:72px; border-radius:50%; display:flex; align-items:center; justify-content:center; font-size:20px; font-weight:800; color:white; margin:0 auto 6px; }}
  .donut-lbl {{ font-size:11px; color:#64748B; font-weight:600; }}
  .donut-val {{ font-size:16px; font-weight:800; color:#1B2A3B; }}
...
<div class="header-bar">
  <div>
    <div class="header-title">🏢 DSI Amendis — Inventaire Informatique Tétouan 2026</div>
    <div class="header-sub">Direction des Systèmes d'Information · Tableau de bord interactif</div>
    <div class="stat-row">
      <span class="stat-pill">📅 {datetime.now().strftime("%d/%m/%Y")}</span>
      <span class="stat-pill">🏙️ Site Tétouan</span>
      <span class="stat-pill">🏢 {nb_dir_d} Directions</span>
      <span class="stat-pill">🖥️ {total_pc} PC actifs</span>
    </div>
  </div>
...
<div class="grid g4" style="margin-bottom:16px;">
  <div class="kpi-card" style="border-left-color:#C8102E;">
    <div style="font-size:28px; margin-bottom:4px;">🖥️</div>
    <div class="kpi-big" style="color:#C8102E;">{total_pc}</div>
    <div class="kpi-lbl">Total PC</div>
    <div class="kpi-sub">Bureau {bureau_n} · Portable {portable_n}</div>
  </div>
  <div class="kpi-card" style="border-left-color:#E67E22;">
    <div style="font-size:28px; margin-bottom:4px;">🔴</div>
    <div class="kpi-big" style="color:#E67E22;">{urgent_n}</div>
    <div class="kpi-lbl">Urgents à Remplacer</div>
    <div class="kpi-sub">{round(urgent_n/total_pc*100) if total_pc else 0}% du parc</div>
  </div>
  <div class="kpi-card" style="border-left-color:#27AE60;">
    <div style="font-size:28px; margin-bottom:4px;">🟢</div>
    <div class="kpi-big" style="color:#27AE60;">{ok_n}</div>
    <div class="kpi-lbl">Opérationnels</div>
    <div class="kpi-sub">{round(ok_n/total_pc*100) if total_pc else 0}% du parc</div>
  </div>
  <div class="kpi-card" style="border-left-color:#9B59B6;">
    <div style="font-size:28px; margin-bottom:4px;">🖨️</div>
    <div class="kpi-big" style="color:#9B59B6;">{imp_n}</div>
    <div class="kpi-lbl">Imprimantes</div>
    <div class="kpi-sub">Chromebooks: {chrom_nn}</div>
  </div>
</div>

<!-- LIGNE 2: État + Ancienneté -->
<div class="grid g23" style="margin-bottom:16px;">
  <div class="card">
    <div class="card-title">📊 État du Parc PC par Priorité</div>
    <div class="donut-wrap">
      <div class="donut-item">
        <div class="donut-circle" style="background:#C8102E;">{urgent_n}</div>
        <div class="donut-lbl">🔴 Urgents</div>
      </div>
      <div class="donut-item">
        <div class="donut-circle" style="background:#E67E22;">{prio_n}</div>
        <div class="donut-lbl">🟠 Prioritaires</div>
      </div>
      <div class="donut-item">
        <div class="donut-circle" style="background:#27AE60;">{ok_n}</div>
        <div class="donut-lbl">🟢 Opérationnels</div>
      </div>
    </div>
    <div style="margin-top:16px;">
      <div style="display:flex; justify-content:space-between; font-size:12px; color:#64748B; margin-bottom:6px;">
        <span>Urgents</span><span style="font-weight:700; color:#C8102E;">{round(urgent_n/total_pc*100) if total_pc else 0}%</span>
      </div>
      <div style="background:#F2F2F2; border-radius:6px; height:8px; margin-bottom:8px;">
        <div style="background:#C8102E; width:{round(urgent_n/total_pc*100) if total_pc else 0}%; height:100%; border-radius:6px;"></div>
      </div>
      <div style="display:flex; justify-content:space-between; font-size:12px; color:#64748B; margin-bottom:6px;">
        <span>Opérationnels</span><span style="font-weight:700; color:#27AE60;">{round(ok_n/total_pc*100) if total_pc else 0}%</span>
      </div>
      <div style="background:#F2F2F2; border-radius:6px; height:8px;">
        <div style="background:#27AE60; width:{round(ok_n/total_pc*100) if total_pc else 0}%; height:100%; border-radius:6px;"></div>
      </div>
    </div>
  </div>
  <div class="card">
    <div class="card-title">📅 Chiffres Clés</div>
    <div style="display:flex; flex-direction:column; gap:12px; margin-top:4px;">
      <div style="display:flex; justify-content:space-between; align-items:center; padding:10px 14px; background:#F8FAFC; border-radius:10px; border-left:4px solid #C8102E;">
        <span style="font-size:13px; color:#64748B;">Âge moyen du parc</span>
        <span style="font-size:18px; font-weight:800; color:#C8102E;">{avg_age_d} ans</span>
      </div>
      <div style="display:flex; justify-content:space-between; align-items:center; padding:10px 14px; background:#F8FAFC; border-radius:10px; border-left:4px solid #E67E22;">
        <span style="font-size:13px; color:#64748B;">PC &gt; 10 ans</span>
        <span style="font-size:18px; font-weight:800; color:#E67E22;">{old_n} ({round(old_n/total_pc*100) if total_pc else 0}%)</span>
      </div>
      <div style="display:flex; justify-content:space-between; align-items:center; padding:10px 14px; background:#F8FAFC; border-radius:10px; border-left:4px solid #2980B9;">
        <span style="font-size:13px; color:#64748B;">Directions couvertes</span>
        <span style="font-size:18px; font-weight:800; color:#2980B9;">{nb_dir_d}</span>
      </div>
      <div style="display:flex; justify-content:space-between; align-items:center; padding:10px 14px; background:#F8FAFC; border-radius:10px; border-left:4px solid #27AE60;">
        <span style="font-size:13px; color:#64748B;">Chromebooks</span>
        <span style="font-size:18px; font-weight:800; color:#27AE60;">{chrom_nn}</span>
      </div>
    </div>
  </div>
</div>

<!-- LIGNE 3: Directions table + Processeurs + RAM -->
<div class="grid g3" style="margin-bottom:16px;">
  <div class="card" style="grid-column: span 1;">
    <div class="card-title">🏢 PC par Direction</div>
    <div style="overflow-x:auto; max-height:320px; overflow-y:auto;">
      <table>
        <thead><tr>
          <th>Direction</th><th style="text-align:center;">PC</th>
          <th>Part</th><th style="text-align:center;">🔴</th>
        </tr></thead>
  ...
  <div class="card">
    <div class="card-title">⚙️ Processeurs</div>
    {proc_html if proc_html else '<p style="color:#94A3B8; font-size:13px;">Données non disponibles</p>'}
  </div>
  <div class="card">
    <div class="card-title">💾 Distribution RAM</div>
    {ram_html if ram_html else '<p style="color:#94A3B8; font-size:13px;">Données non disponibles</p>'}
  </div>
</div>

<!-- LIGNE 4: Urgents list -->
{f'''<div class="card" style="margin-bottom:16px;">
  <div class="card-title">🔴 PC Urgents à Remplacer (Top {min(urgent_n,8)})</div>
  {urg_list_html}
</div>''' if urgent_n > 0 else ''}

<!-- FOOTER -->
<div style="background:#1B2A3B; border-radius:12px; padding:14px 22px; display:flex; justify-content:space-between; align-items:center; margin-top:4px;">
  <span style="font-size:12px; color:#64748B;">DSI — Direction des Systèmes d'Information · Site Tétouan · Inventaire 2026</span>
  <span style="font-size:12px; color:#C8102E; font-weight:600;">Généré le {datetime.now().strftime("%d/%m/%Y à %H:%M")}</span>
</div>
...
"""
    ...
    st.components.v1.html(dashboard_html, height=1800, scrolling=True)
